...
switch ($_GET['page']) {
	...
	case 'affiliates':
		...
		if($sql->rowCount() == 0) {
			$affiliates = array(
				...
				'lifetime_earnings' => 0,
				'available' => 0,
				'level' => "<b style='color:#965A38'><i class='fa fa-star'></i> Бронза</b> (1 монета за 300 поставленных)",
				'depositors' => "0/50 до серебра",
				'code' => '(у вас нет промокода)'
				);
		} else {
			$row = $sql->fetch();
			...
			if($count < 50) {
				$affiliates['level'] = "<b style='color:#965A38'><i class='fa fa-star'></i> Bronze</b> (1 coin per 300 bet)";
				$affiliates['depositors'] = $count."/50 to Silver";
				$s = 300;
			} elseif($count > 50) {
				$affiliates['level'] = "<b style='color:#A9A9A9'><i class='fa fa-star'></i> Silver</b> (1 coin per 200 bet)";
				$affiliates['depositors'] = $count."/200 to Gold";
				$s = 200;
			} elseif($count > 200) {
				$affiliates['level'] = "<b style='color:#FFD700'><i class='fa fa-star'></i> Gold</b> (1 coin per 100 bet)";
	...
	case 'collect':
		if(!$user) exit(json_encode(array('success'=>false, 'error'=>'Вам необходимо авторизоватся чтобы получить монеты.')));
		$sql = $db->query('SELECT * FROM `users` WHERE `referral` = '.$db->quote($user['steamid']));
		$reffersN = $sql->fetchAll();
		$count = 0;
		$collect_coins = 0;
	...
	case 'redeem':
		...
		if($user['referral'] != '0') exit(json_encode(array('success'=>false, 'error'=>'You almost enter code', 'code'=>$user['referral'])));
	  ...
	  case 'deposit_js':
        ...
        foreach ($inv['rgInventory'] as $key => $value) {
            ...
            for ($i = 0; $i < count(assetids); $i++) {
                if($kek != 0) break;
                if ((preg_match('/(Sticker|Souvenir)/', $name) and $assetids[$i] == $value['id']) || ($prices['response']['items'][$name]['value']*10 < $min and $assetids[$i] == $value['id'])) {
                    $kek++;
                }
            }
        }

        if ($kek != 0) exit(json_encode(array('success' => false, 'error' => 'Incorrect item !')));
	...
	if(!$user) exit(json_encode(array('success'=>false, 'error'=>'You must login to access the confirm.')));
		...
		if(($out['success'] == true) && ($out['action'] == 'accept') && ($row['status'] != 1)) {
			...
			exit(json_encode(array('success'=>false, 'error'=>'Ошибка: Вам уже начислены монеты')));
		}
		exit(json_encode($out));
		break;
	...
	case 'get_bank_safe':
		...
		$g = curl('https://www.google.com/recaptcha/api/siteverify?secret=6LeQNx8TAAAAAAaFodLUlb567NiCZzwGNWi-wO0X&response='.$_GET['g-recaptcha-response']);
		...
		if($g['success'] == true) {
			...
			$prices = json_decode($prices, true);
			while ($row = $sql->fetch()) {
				$array['items'][] = array('botid'=>$row['botid'],'img'=>'http://steamcommunity-a.akamaihd.net/economy/image/'.$row['img'],'name'=>$row['market_hash_name'],'assetid'=>$row['id'],'price'=>$prices['response']['items'][$row['market_hash_name']]['value']*10+100,'reject'=>'unknown items');
			}
			exit(json_encode($array));
		}
		break;
	...
	case 'withdraw_js':
		...
		if(count($items) > 1) {
			...
			$out = array('success'=>false,'error'=>'You need more coins!');
		} else {
			reset($items);
			$bot = key($items);
